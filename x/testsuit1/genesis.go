...
import (
	...
	"github.com/user/chaina/x/testsuit1/keeper"
...
func InitGenesis(ctx sdk.Context, k keeper.Keeper, genState types.GenesisState) {
	...
	// Set all the sendData
	for _, elem := range genState.SendDataList {
		k.SetSendData(ctx, *elem)
	}
...
func ExportGenesis(ctx sdk.Context, k keeper.Keeper) *types.GenesisState {
	...
	// Get all sendData
	sendDataList := k.GetAllSendData(ctx)
	for _, elem := range sendDataList {
		elem := elem
		genesis.SendDataList = append(genesis.SendDataList, &elem)
	}
