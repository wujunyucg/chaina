    ...
    actions: {
        ...
        async QuerySendData({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySendData', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QuerySendDataAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).querySendDataAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySendDataAll', payload: { options: { all }, params: { ...key }, query } });
